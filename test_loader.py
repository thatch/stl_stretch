...
class StlTestHelpers(object):
    def assertComparesIdentical(self, stl1, stl2):
        ...
        for k, v in stl1._vertex_cache.items():
...
class TestLoader(unittest.TestCase):
    ...
    def test_cube_loader_bad_binary(self):
        # These should not start with b'solid' but sometimes do
        m = stl.Mesh("files/cube-bad-binary.stl")
        self.assertEqual(len(m.facets), 12)
        self.assertEqual(len(m._vertex_cache), 8)
